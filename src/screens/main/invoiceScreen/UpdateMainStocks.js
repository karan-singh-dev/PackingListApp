...
const UpdateMainStocks = ({ navigation }) => {
...
const handleFilePick = async () => {
  try {
    ...
    // Convert file URI to path (RNFS expects no file:// prefix)
    ...
    const headerRow = worksheet.getRow(1).values.slice(1); // skip first empty index
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setProceeded(false)
      navigation.navigate('MainStockList');
    } catch (err) {
...
export default UpdateMainStocks;
